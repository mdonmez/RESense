...
2. Run three verified cycles covering fan auto/max/custom, operation modes,
   keyboard lighting/brightness/timeout/Sticky Keys/Win/Menu lock, LCD
   Overdrive, and DTS sound presets when the current output supports them.
3. Restore the captured configuration and verify it.

The matrix waits one second after every mutation so vendor-service and
Windows-session state have time to settle before the next operation or
readback.

Live fan temperature and RPM are not compared during restoration because they
naturally change. Fan mode and control percentages, keyboard state, operation
mode, display state, and sound preset are compared.
