...
| Sticky keys | `live`, Windows `SystemParametersInfo` | admin pipe `cmd 2`; HKLM compatibility sync is NitroSense-facing only and not a reliable cross-session truth source | partial: NitroSense advanced setting follows HKLM | Windows state plus NitroSense toggle | live | no additional supported read/write gap |
...
| Sound preset (supported path) | `live`, admin pipe `cmd 13` on the validated DTS path for internal speakers and wired 3.5 mm Realtek output; HKLM `DTS_Audio_Support` gates support | admin pipe `cmd 14`; validated as effectively shared/global across active sessions on this machine | yes | NitroSense sound page on internal speakers and wired headphones | validated | Bluetooth/non-DTS output remains unsupported |
...
| Sound backend `auto` | resolves to DTS for the validated internal-speaker and wired 3.5 mm surface | non-DTS paths are unsupported | documented supported behavior |
