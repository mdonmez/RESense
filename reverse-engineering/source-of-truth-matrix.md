...
| Sticky keys | `live`, Windows `SystemParametersInfo` | admin pipe `cmd 2` plus HKLM compatibility sync | partial: NitroSense advanced setting follows HKLM | Windows state plus NitroSense toggle | live | multi-session admin-agent behavior still worth documenting |
...
| Operation mode | `live`, service `cmd 34/query 11`; HKLM mode code is a mirror | service `cmd 30` plus HKLM sync; WhisperMode best-effort side effect over admin pipe | yes | NitroSense mode cards, NVIDIA App for WhisperMode | validated | explain why `u32::MAX` from `cmd 30` is accepted |
...
| Sound preset (supported path) | `live`, admin pipe `cmd 13` on DTS/internal-speaker path; HKLM `DTS_Audio_Support` gates support | admin pipe `cmd 14` | yes | NitroSense sound page on internal speakers | validated | wired `3.5 mm` Waves path deferred |
